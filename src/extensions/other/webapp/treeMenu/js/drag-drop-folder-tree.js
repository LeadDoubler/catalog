	...
	************************************************************************************************************/
	...
	function JSDragDropTree()
	{
		...
		this.imageFolder = '../treeMenu/images/';
				...
				if(JSTreeObj.insertAsSub){
					...
					if(JSTreeObj.dragNode_destination.nextSibling){
						...
						JSTreeObj.dragNode_destination.parentNode.appendChild(JSTreeObj.dragNode_source);
                            ...
                            movePage(JSTreeObj.dragNode_source.id,JSTreeObj.dragNode_destination.id);
				...
				while(li){
					if(li.id){
                        ...
                        if(numericID!='999999999'){
                            saveString = saveString + '<li status=\"'+li.getAttribute('status')+'\" id=\"'+numericID+'\" menuname=\"'+li.getAttribute('menuname')+'\" title=\"'+li.getAttribute('title')+'\"><a href=\"'+li.getElementsByTagName('A')[0].href+'\">'+li.getAttribute('menuname')+'</a>';
                            var ul = li.getElementsByTagName('UL');
						    if(ul.length>0){
                               saveString = saveString + '\n<ul>  ';
                               saveString = this.getNodeOrders(ul[0],saveString)+'</ul>\n';
                            }
                            saveString = saveString + '</li>';
                       	}
                        else{
                            var ul = li.getElementsByTagName('UL');
						    if(ul.length>0){
