...
public abstract class HibernateProvider
{
	 ...
	 */
	abstract public SessionFactory getSessionFactory(Class<? extends Object> forClass);

	/**
	 * Gets the Configuration used to build the SessionFactory associated with the class specified. 
	 */
	abstract public Configuration getConfiguration(Class<? extends Object> forClass);
	
	/**
	 * Gets a Session from the SessionFactory associated with the class specified.
	 */
	abstract public Session getSession(Class<? extends Object> forClass);
	...
	public void commit(Class<? extends Object> forClass) { commit(getSession(forClass)); }
	...
	public void rollback(Class<? extends Object> forClass) { rollback(getSession(forClass)); }
	public void rollback(Session session) { throw new UnsupportedOperationException("Not implemented"); }
	
	/**
	 * Attempt to perform a recursive save (or update) so that we don't have to save every
	 * different entity in a graph separately.
	 * 
	 * @param entity
	 */
	public void recursiveSave(Object entity)
	{
		recursiveSave(entity, getSession(entity.getClass()), new HashSet<Object>());
	}
	@SuppressWarnings("unchecked")
	protected void recursiveSave(Object entity, Session session, Set<Object> savedEntities)
	{
		if ((getSessionFactory(entity.getClass()) == null) || !savedEntities.add(entity))
			return;
		...
		{
			Map.Entry mapEntry = (Map.Entry) entity;
			
			recursiveSave(mapEntry.getKey());
			recursiveSave(mapEntry.getValue());
			
			return;
		}

		// Try to save all properties with a public getter
		for (Method getter : entity.getClass().getMethods())
		{
			// only attempt to save public properties
			if (((getter.getModifiers() & Modifier.PUBLIC) != Modifier.PUBLIC) ||
			...
			try
			{
				...
				if (value != null)
					recursiveSave(value , session, savedEntities);
			}
			catch (Exception e) { /* Well, we tried */ }
		}

		// Save parent after children
